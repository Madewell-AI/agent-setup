...
Read the template at ~/agent-setup/codex/AGENTS.md.template. Replace all {{VARIABLE}} placeholders with values from config.json:
...
Also create ~/.codex/AGENTS.md as a global config (same content).

## Step 3: Install Hooks

Copy ~/agent-setup/codex/hooks.json to ~/.codex/hooks.json
...
Note: Codex CLI also has a built-in memory system that will auto-populate ~/.codex/memories/memory_summary.md over time. Both systems complement each other.
...
Note: The cron system auto-detects the engine. If claude is not found, it falls back to codex.
...
```
bash ~/.agent/scripts/hooks/session-start.sh
```
...
- Remind them that AGENTS.md includes an instruction to run auto-recall on every message — this compensates for the lack of a UserPromptSubmit hook
